...
//-------------------------------------------------------------------------------
        ...
        //Select place in Google Drive
        foreach (kmgoogle_build_select_places($COURSE->id) as $name) {
            $options[$name] = $name;
        }
        $options = array(''=>get_string('choose').'...') + $options;
        $mform->addElement('select', 'secondfolder', get_string("kmgoogleplace", "kmgoogle"), $options);
        $mform->addRule('secondfolder', $strrequired, 'required', null, 'client');
        $mform->addHelpButton('secondfolder', 'kmgoogleplace', 'kmgoogle');

        //Standart intro
        $this->standard_intro_elements(get_string('customintro', 'kmgoogle'));

        //Input Source Google url
        $mform->addElement('text', 'sourcegoogleurl', get_string('sourcegoogleurl', "kmgoogle"), array('size'=>'80'));
        $mform->setType('sourcegoogleurl', PARAM_URL);
        $mform->addRule('sourcegoogleurl', null, 'required', null, 'client');

        //Copied Source Google url
        $mform->addElement('text', 'copiedgoogleurl', get_string('copiedgoogleurl', "kmgoogle"), array('size'=>'80', "readonly"=>""));
        if (!empty($CFG->formatstringstriptags)) {
            $mform->setType('copiedgoogleurl', PARAM_TEXT);
        } else {
            $mform->setType('copiedgoogleurl', PARAM_CLEANHTML);
        }
        ...
        //Association Level
        $options = kmgoogle_association_level();
        $mform->addElement('select', 'association', get_string("association", "kmgoogle"), $options);

        //Primary association's name
        $options = array();
        $mform->addElement('select', 'associationname', get_string("associationname", "kmgoogle"), $options);
        ...
        $mform->addElement('checkbox', 'sendtoteacher', get_string('sendtoteacher', 'kmgoogle'));

        //The nature of serving
        $options = array('0' => get_string("course"),
                         '1' => get_string("activity"),
        );
        $mform->addElement('select', 'natureofserving', get_string("natureofserving", "kmgoogle"), $options);

        //Requires students to click
        $options = array('0' => get_string("no"),
                         '1' => get_string("yes"),
        );
        $mform->addElement('select', 'studenttoclick', get_string("studenttoclick", "kmgoogle"), $options);

        //Student consent is required for the submission statement
        $options = array('0' => get_string("no"),
                         '1' => get_string("yes"),
        );
        $mform->addElement('select', 'studentconsent', get_string("studentconsent", "kmgoogle"), $options);

        //Submitting mechanism
        $options = array('0' => get_string("automatic", "kmgoogle"),
                         '1' => get_string("unlimited"),
        );
        $mform->addElement('select', 'submitmechanism', get_string("submitmechanism", "kmgoogle"), $options);

        //Maximum number of attempts
        $mform->addElement('text', 'numberattempts', get_string('numberattempts', 'kmgoogle'));
