...
function kmgoogle_add_instance($kmgoogle) {
    ...
    $newurl = kmgoogle_copy_google_url($kmgoogle->sourcegoogleurl);
    if(!$newurl){
        return 0;
    }
...
function kmgoogle_update_instance($kmgoogle) {
    ...
    if($row->sourcegoogleurl != $kmgoogle->sourcegoogleurl){
        $newurl = kmgoogle_copy_google_url($kmgoogle->sourcegoogleurl);
        if(!$newurl){
            return 0;
        }
...
//    $cm = get_coursemodule_from_instance('kmgoogle', $id);
//    \core_completion\api::update_completion_date_event($cm->id, 'kmgoogle', $id, null);
//
//    $result = true;
//
//    if (! $DB->delete_records("kmgoogle_analysis", array("kmgoogle"=>$kmgoogle->id))) {
//        $result = false;
//    }
//
//    if (! $DB->delete_records("kmgoogle_answers", array("kmgoogle"=>$kmgoogle->id))) {
//        $result = false;
//    }
//
