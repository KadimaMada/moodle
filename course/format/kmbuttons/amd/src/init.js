define(['jquery','format_kmbuttons/slick', 'format_kmbuttons/ajax'], function($, slick, ajax) {
  ...
  function initDefaults(initparams){
    ...
    document.addEventListener('click', function(e){
      let target = e.target;
      while (!target.classList.contains('kmbuttons')) {
  ...
  function catchMouse(e){
    ...
    var target = document.querySelector('div.kmbuttons');
...
// add events to server
  function sendEventToServer(target) {
    const mainBlock = document.querySelector('.kmbuttons[data-userid]');
    if (!mainBlock) return;
