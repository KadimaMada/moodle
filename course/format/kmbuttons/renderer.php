 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_kmbuttons_renderer extends format_topics_renderer
{
...
/*             if ($course->sequential) {
            ...
            $class = 'buttonsection';
            $onclick = 'M.format_kmbuttons.show(' . $section . ',' . $course->id . ')';
        ...
        if ($PAGE->user_is_editing()) {
            $html .= html_writer::tag('div', get_string('editing', 'format_kmbuttons'), ['class' => 'alert alert-warning alert-block fade in']);
        }
        return $html;
    }
    ...
    protected function get_button_section_kadima($course, $sectionvisible)
    {
        ...
        $inline = '';
        $count = 1;
// echo '<pre>'.print_r($course->id,1).'</pre>'; exit();
        if (!$PAGE->user_is_editing()) {
        // start kadima container render
        $html .= html_writer::start_tag('div',['class' => 'container-fluid kmbuttons', 'data-userid' => $USER->id, 'data-courseid' => $course->id]); // don't forget to close it later
...
/*    SG -      hide as nor used
            ...
            */
            ...
            //$html .= html_writer::tag('span', '', ['class' => 'section-icon d-inline-flex p-3 justify-content-center align-items-center '.$sectionnamearr[2], 'style' => "background: url({$this->courserenderer->image_url('label-default', 'format_kmbuttons')}) no-repeat; background-size: cover;"]);  // SG - previouse variant
    ...
    {
        return html_writer::start_tag('ul', ['class' => 'kmbuttons']);
    }
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused)
    {
        ...
        */
        ...
        $csscoloroptions .= "--kmbuttons-course-description: ".$course->course_descr_bg_color.";";
        $csscoloroptions .= "--kmbuttons-section-bg-color: ".$course->section_menu_bg_color.";";
        $csscoloroptions .= "--kmbuttons-section-font-color: ".$course->section_menu_font_color.";";
        $csscoloroptions .= "--kmbuttons-section-icon-color: ".$course->section_menu_icon_color.";";
        $csscoloroptions .= "--kmbuttons-section-menu-arrows-color: ".$course->section_menu_arrows_color.";";
        $csscoloroptions .= "--kmbuttons-section-menu-arrows-color-hover: ".$course->section_menu_arrows_color_hover.";";
        $csscoloroptions .= "--kmbuttons-section-menu-info-icon-color: ".$course->section_menu_info_icon_color.";";
        $csscoloroptions .= "--kmbuttons-section-menu-info-icon-color-hover: ".$course->section_menu_info_icon_color_hover.";";
        $csscoloroptions .= "--kmbuttons-section-active-bg: ".$course->selected_section_bg_color.";";
        $csscoloroptions .= "--kmbuttons-section-active-font-color: ".$course->selected_section_font_color.";";
        $csscoloroptions .= "--kmbuttons-section-active-icon-color: ".$course->selected_section_icon_color.";";
        $csscoloroptions .= "--kmbuttons-label-bg: ".$course->label_menu_bg_color.";";
        $csscoloroptions .= "--kmbuttons-label-font-color: ".$course->label_menu_font_color.";";
        $csscoloroptions .= "--kmbuttons-label-icon-color: ".$course->label_menu_icon_color.";";
        $csscoloroptions .= "--kmbuttons-label-controls-color: ".$course->label_menu_arrows_color.";";
        $csscoloroptions .= "--kmbuttons-label-active-bg: ".$course->selected_label_bg_color.";";
        $csscoloroptions .= "--kmbuttons-label-active-font-color: ".$course->selected_label_font_color.";";
        $csscoloroptions .= "--kmbuttons-label-active-icon-color: ".$course->selected_label_icon_color.";";
        $csscoloroptions .= "}";
        ...
        if (!$PAGE->user_is_editing()) {
          ...
          if (!$PAGE->user_is_editing()) {
            ...
            echo html_writer::end_tag('div'); // container-fluid kmbuttons ends here (starts in get_button_section_kadima)
          }
        ...
        echo html_writer::tag('style', '.course-content ul.kmbuttons #section-'.$sectionvisible.' { display: block; }');
        // if (!$PAGE->user_is_editing()) {
        //     $PAGE->requires->js_init_call('M.format_kmbuttons.init', [$course->numsections]);
        // }
        // ==============================================================================
        // don't needed - implemented above
        //echo $this->course_format_kmbuttons_design($course, $section);
    }
    ...
    public function labels_list($course, $section) {
        ...
        foreach ($labels as $modnum => $content) {
            ...
            // if (empty($content[2])) {
            //     $liconStyle = 'background: url('.$this->courserenderer->image_url('label-default', 'format_kmbuttons').') no-repeat; background-size: cover; padding:14px;';
            //     $liconClass = '';
            // } else {
              if(preg_match('/fa-/im', $content[2]) === 1) {
                ...
                $liconStyle = 'background: url('.$this->courserenderer->image_url($content[2], 'format_kmbuttons').') no-repeat; background-size: cover; padding:14px;';
                $liconClass = '';
              }
            // }
