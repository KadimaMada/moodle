...
class classAjax {
    ...
    private function geteventonclick() {
        ...
        if ($fbusid = $DB->get_record('format_kmbuttons_userstate', array ('userid' => $userid, 'courseid' => $courseid))) {
            $record->id = $fbusid->id;
            $DB->update_record('format_kmbuttons_userstate', $record);
        } else {
            $DB->insert_record('format_kmbuttons_userstate', $record);
        }
        

        // trigger event, that label or section were viewed 
        $event = \format_kmbuttons\event\log_label_clicked::create(array(
            'context' => $context,
            'userid' => $userid,
            'other' => array (
                'modtype' => $modtype,
